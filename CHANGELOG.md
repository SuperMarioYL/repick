...
- **fix: CI green again** — `ci.yml` pinned Bun 1.1 but the lockfile is `lockfileVersion: 2` (Bun ≥1.2), so every CI run since v0.1.0 failed at `bun install --frozen-lockfile` with `InvalidLockfileVersion`. CI now pins 1.4 (1.2.x cannot parse the lockfile either — verified: CI run 35382531157 on Bun 1.2.23 still failed); `engines.bun` and the README install floor moved to 1.4+ to match what the lockfile actually requires.
